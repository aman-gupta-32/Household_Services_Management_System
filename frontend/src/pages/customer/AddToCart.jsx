...
function AddToCart() {
  ...
  const checkSlot = async(updatedBooking) => {

    console.log("Booking data : ",updatedBooking)
    const resp = await slotChecking(updatedBooking);


    if (resp.message !== "confirm booking") {
      console.log("in the confirm 2"+resp.message);
  ...
  const handleConfirm = async () => {
    ...
    const updatedBooking = {
      ...bookingData,
      bookingSlot: Number(selectedSlot),
      date: selectedDate,
      servicesId: selectedServiceId,
      customersId: localStorage.getItem("userId"),
      sname: serviceName,
    ...
    let storedBookings = JSON.parse(localStorage.getItem("bookingData")) || [];
